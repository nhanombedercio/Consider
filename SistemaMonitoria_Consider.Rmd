...
valueBox(sum(Desistentes_F$Quantidade) ,caption="TOTAL DE DESISTENTES",icon ="fas fa-venus", color = "#9442D4")
```
...
# POR REGIAO {data-navmenu="RESUMO"}
...
Fe_G= Fe %>% na.omit() %>%
  filter(Regiao %in% input$RegiaoProdutor)
#Quantos produtores temos em Gorongosa do sexo Feminino}
valueBox(sum(Fe_G$Quantidade),caption="PRODUTORAS DO SEXO FEMININO",icon = "fas fa-venus", color = "#9442D4")
})
```
...
Plider_F_G= dados  %>% filter(Regiao%in% input$RegiaoProdutor) %>% filter(ProdutorLider=="SIM",nrSessao==1, Sexo=="FEMININO")  %>%
  ...
  summarise(Quantidade = n()) 
#Quantos produtores temos em Gorongosa do sexo Feminino
valueBox(sum(Plider_F_G$Quantidade),caption="PRODUTORAS LIDERES DO SEXO FEMININO",icon = "fas fa-venus", color = "#9442D4")})
```
...
valueBox(sum(Desistentes_F_G$Quantidade),caption="TOTAL DE DESISTENTES",icon ="fas fa-venus", color = "#9442D4")})
```
...
# Presença de parceiros nas sessões {data-navmenu="PARCEIROS"}
...
### Percentagem de parceiros do sexo feminino que realizam todas as photovoice
...
# Lista das instituições parceiras {data-navmenu="PARCEIROS"}
