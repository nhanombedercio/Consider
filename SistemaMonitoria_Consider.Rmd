...
hr(),
("Para mostrar os dados deves selecionar na caixinha em baixo uma ou duas regiões"),
hr(),

  selectInput("RegiaoProdutor", label =  ("Selecione a Região"), 
               choices = unique(dados$Regiao), multiple = TRUE), 
  hr()  
)
```
...
fluidPage(radioButtons("REGIAOParc", label =  ("Selecione a Regiao"), 
               choices = unique(parceiros$Regiao), selected = "GORONGOSA")
...
fluidPage(
  ("NESSE PAINEL APRESENTAMOS OS DADOS DAS SESSÕES COMUNITARIAS"),
   hr(),
  ("EM BAIXO APRESENTAMOS AS OPÇÕES DE FILTRAGEM"),
  hr(),
  ...
  selectInput("checkNrsessao", label =("Selecione numero da sessao"), 
               choices =unique(comunidaderesumo$Nrsessao), multiple = TRUE),
   
  selectInput("regiaocheckxx1", label =("Selecione pelo menos uma Região"), 
               choices =unique(comunidaderesumo$Regiao), multiple = TRUE))
```
...
filter(Nrsessao %in% input$checkNrsessao, Regiao %in% input$regiaocheckxx1))  %>% formatStyle(
